...
const createSupervisionRequest = catchError(
  async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    ...
    const existingRequests =
      await supervisionRequestService.getBachelorsSupervisionRequestsToSupervisor(
        data.bachelor_id,
        data.supervisor_id
      );

    if (existingRequests.some(request => request.status !== SupervisionRequestStatus.rejected))
      return next(
